...
Fetches 26.3 Rx'd scores for Men and Women and saves to CSV.
"""
...
OUTPUT_CSV = os.path.join(DATA_DIR, "26.3_scores.csv")
...
def page_has_scores(rows):
    ...
    return any(
        row.get("scores") and len(row["scores"]) > SCORE_INDEX
        and row["scores"][SCORE_INDEX].get("scoreDisplay", "").strip()
        for row in rows
    )
